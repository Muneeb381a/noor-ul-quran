    ...
    <div onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.85rem' }}>
        <div style={{
          ...
          color: isActive ? '#030A05' : 'rgba(255,255,255,0.3)',
        }}>{step.step}</div>
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontWeight: 700, fontSize: '0.88rem', color: isActive ? color : 'rgba(255,255,255,0.8)' }}>{step.name}</p>
          {step.position && <p style={{ margin: '0.1rem 0 0', fontSize: '0.68rem', color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>{step.position}</p>}
          {isActive && (
            <div style={{ marginTop: '0.85rem' }}>
              {step.arabic && (
                <div style={{ background: `${color}0A`, borderRadius: 12, padding: '0.9rem 1rem', marginBottom: '0.65rem', textAlign: 'right', border: `1px solid ${color}20` }}>
                  <p style={{ fontFamily: '"Noto Naskh Arabic","Amiri Quran",serif', fontSize: 'clamp(1.05rem,2.5vw,1.3rem)', lineHeight: 2.2, color: 'rgba(255,255,255,0.92)', margin: 0, direction: 'rtl' }}>{step.arabic}</p>
                </div>
              )}
              {step.transliteration && (
                <p style={{ fontStyle: 'italic', color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', lineHeight: 1.7, margin: '0 0 0.5rem' }}>{step.transliteration}</p>
              )}
              {step.translation && (
                <div style={{ background: 'rgba(46,125,50,0.1)', borderRadius: 8, padding: '0.5rem 0.75rem', borderLeft: '3px solid #52B788', marginBottom: '0.5rem' }}>
                  <p style={{ margin: 0, fontSize: '0.82rem', color: 'rgba(255,255,255,0.7)', lineHeight: 1.65 }}>
              ...
              {step.urdu && (
                <div style={{ background: 'rgba(106,27,154,0.1)', borderRadius: 8, padding: '0.5rem 0.75rem', borderRight: '3px solid #B39DDB', textAlign: 'right', marginBottom: '0.5rem' }}>
                  <p style={{ margin: 0, fontSize: '0.88rem', color: 'rgba(255,255,255,0.7)', lineHeight: 3.4, fontFamily: '"Noto Nastaliq Urdu",serif', direction: 'rtl', wordSpacing: '3px' }}>
                    <span style={{ fontWeight: 700, color: '#B39DDB', marginLeft: 4 }}>اردو:</span>{step.urdu}
                  </p>
                </div>
              )}
              {step.desc && (
                <div style={{ background: 'rgba(201,168,76,0.08)', borderRadius: 8, padding: '0.5rem 0.75rem', borderLeft: `3px solid rgba(201,168,76,0.5)` }}>
                  <p style={{ margin: 0, fontSize: '0.79rem', color: 'rgba(255,255,255,0.55)', lineHeight: 1.7 }}>{step.desc}</p>
                </div>
              )}
            </div>
          )}
        </div>
        <span style={{ color: isActive ? color : 'rgba(255,255,255,0.2)', fontSize: '0.9rem', flexShrink: 0, marginTop: '0.1rem' }}>{isActive ? '▲' : '▼'}</span>
      </div>
    </div>
  );
}
...
export default function NamazPage() {
  ...
  const pColor = PRAYER_COLORS[activePrayer]?.color || '#52B788';
    ...
    <div style={{ minHeight: '100vh', background: '#030A05' }}>
      ...
      <div style={{ background: 'linear-gradient(150deg,#060E09,#0D2818,#1B4332)', padding: '3.5rem 1.5rem 2.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative' }}>
          ...
          <p style={{ fontFamily: '"Noto Nastaliq Urdu",serif', fontSize: '1.1rem', color: 'rgba(201,168,76,0.7)', direction: 'rtl', margin: '0 0 0.75rem', lineHeight: 3.4, wordSpacing: '3px' }}>نماز پڑھنے کا طریقہ</p>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.9rem', maxWidth: 560 }}>{namazData.intro}</p>
        </div>
      </div>

      {/* Section tabs */}
      <div style={{ position: 'sticky', top: 68, zIndex: 10, background: 'rgba(3,10,5,0.95)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(201,168,76,0.12)', padding: '0.65rem 1.5rem' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', gap: '0.5rem', overflowX: 'auto' }}>
          {SECTION_TABS.map(tab => (
            <button key={tab} onClick={() => setActiveSection(tab)} style={{
              ...
              borderColor: activeSection === tab ? 'rgba(201,168,76,0.5)' : 'rgba(255,255,255,0.1)',
              background: activeSection === tab ? 'linear-gradient(135deg,#C9A84C,#E9C46A)' : 'rgba(255,255,255,0.04)',
              color: activeSection === tab ? '#030A05' : 'rgba(255,255,255,0.5)',
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
        ...
        {activeSection === 'Overview' && (
          <div>
            <h2 style={{ fontFamily: '"Cinzel",serif', fontSize: '1.1rem', color: 'rgba(201,168,76,0.85)', marginBottom: '1.5rem' }}>5 Daily Prayers — الصلوات الخمس</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
              {namazData.prayers.map(p => {
                const pc = PRAYER_COLORS[p.id];
                return (
                  <div key={p.id} style={{ background: 'rgba(255,255,255,0.025)', borderRadius: 18, border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '1rem 1.5rem' }}>
                      <p style={{ margin: '0 0 0.6rem', fontSize: '0.84rem', color: 'rgba(255,255,255,0.55)', lineHeight: 1.7 }}>{p.desc}</p>
        ...
        {activeSection === 'Wudu' && (
          <div>
            <div style={{ background: 'linear-gradient(135deg,#020D22,#0D2857)', borderRadius: 20, padding: '1.5rem', marginBottom: '1.5rem', border: '1px solid rgba(100,181,246,0.25)' }}>
              ...
              <p style={{ fontFamily: '"Noto Naskh Arabic",serif', color: 'rgba(255,255,255,0.5)', fontSize: '1.1rem', margin: '0 0 0.6rem', direction: 'rtl' }}>الوضوء</p>
              <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.84rem', margin: 0, lineHeight: 1.6 }}>{namazData.wudu.intro}</p>
            </div>
        ...
        {activeSection === 'How to Pray' && (
          <div>
              ...
              {namazData.prayers.map(p => {
                ...
                return (
                  <button key={p.id} onClick={() => { setActivePrayer(p.id); setActiveStep(null); }} style={{
                    ...
                    borderColor: isAct ? `${pc.color}80` : 'rgba(255,255,255,0.1)',
                    background: isAct ? `${pc.color}1A` : 'rgba(255,255,255,0.04)',
                    color: isAct ? pc.color : 'rgba(255,255,255,0.5)',
            ...
            <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.35)', marginBottom: '1rem', background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: '0.65rem 0.9rem', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            {namazData.after_prayer.duas.map((dua, i) => {
              ...
              return (
                <div key={i} onClick={() => setActiveAfterStep(isOpen ? null : i)} style={{
                  background: isOpen ? 'rgba(82,183,136,0.06)' : 'rgba(255,255,255,0.03)',
                  borderRadius: 16,
                  border: `1.5px solid ${isOpen ? 'rgba(82,183,136,0.35)' : 'rgba(255,255,255,0.08)'}`,
                  padding: '0.9rem 1.1rem', marginBottom: '0.6rem', cursor: 'pointer', transition: 'all 0.2s',
                }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                      <div style={{ width: 32, height: 32, borderRadius: 9, background: isOpen ? 'linear-gradient(135deg,#1B4332,#2D6A4F)' : 'rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 800, color: isOpen ? '#C9A84C' : 'rgba(255,255,255,0.3)', flexShrink: 0 }}>{i + 1}</div>
                      <p style={{ margin: 0, fontWeight: 700, fontSize: '0.85rem', color: isOpen ? 'rgba(82,183,136,0.9)' : 'rgba(255,255,255,0.75)' }}>
                        {dua.transliteration.split('(')[0].trim()}
                      </p>
                    </div>
                    <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.8rem' }}>{isOpen ? '▲' : '▼'}</span>
                  </div>
                  {isOpen && (
                    <div style={{ marginTop: '0.9rem' }}>
                      <div style={{ background: 'rgba(82,183,136,0.07)', borderRadius: 12, padding: '0.9rem 1rem', marginBottom: '0.65rem', textAlign: 'right', border: '1px solid rgba(82,183,136,0.18)' }}>
                        <p style={{ fontFamily: '"Noto Naskh Arabic","Amiri Quran",serif', fontSize: 'clamp(0.95rem,2vw,1.2rem)', lineHeight: 2.2, color: 'rgba(255,255,255,0.92)', margin: 0, direction: 'rtl' }}>{dua.arabic}</p>
                      </div>
                      <p style={{ fontStyle: 'italic', color: 'rgba(255,255,255,0.35)', fontSize: '0.79rem', margin: '0 0 0.5rem' }}>{dua.transliteration}</p>
                      <div style={{ background: 'rgba(46,125,50,0.1)', borderRadius: 8, padding: '0.5rem 0.75rem', borderLeft: '3px solid #52B788', marginBottom: '0.5rem' }}>
                        <p style={{ margin: 0, fontSize: '0.8rem', color: 'rgba(255,255,255,0.65)', lineHeight: 1.65 }}><span style={{ fontWeight: 700, color: '#52B788', marginRight: 4 }}>EN:</span>{dua.translation}</p>
                      </div>
                      <div style={{ background: 'rgba(106,27,154,0.1)', borderRadius: 8, padding: '0.5rem 0.75rem', borderRight: '3px solid #B39DDB', textAlign: 'right' }}>
                        <p style={{ margin: 0, fontSize: '0.88rem', color: 'rgba(255,255,255,0.65)', lineHeight: 3.4, fontFamily: '"Noto Nastaliq Urdu",serif', direction: 'rtl', wordSpacing: '3px' }}><span style={{ fontWeight: 700, color: '#B39DDB', marginLeft: 4 }}>اردو:</span>{dua.urdu}</p>
                      </div>
                      {dua.note && (
                        <div style={{ marginTop: '0.5rem', background: 'rgba(201,168,76,0.08)', borderRadius: 8, padding: '0.45rem 0.75rem', borderLeft: '3px solid rgba(201,168,76,0.5)' }}>
                          <p style={{ margin: 0, fontSize: '0.76rem', color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>⭐ {dua.note}</p>
                        </div>
                      )}
                    </div>
                  )}
      ...
      <div style={{ background: 'linear-gradient(135deg,#060E09,#1B4332)', padding: '2.5rem 1.5rem', textAlign: 'center' }}>
        ...
        <Link to="/learn/janaza" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.65rem 1.4rem', background: 'rgba(201,168,76,0.15)', border: '1px solid rgba(201,168,76,0.35)', borderRadius: 99, color: '#C9A84C', fontSize: '0.82rem', fontWeight: 700, textDecoration: 'none' }}>
