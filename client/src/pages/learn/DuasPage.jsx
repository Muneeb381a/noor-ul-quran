...
function DuaCard({ dua, isFav, onFav }) {
  ...
  const meta = CAT_META[dua.category] || { icon: '✨', color: '#1B4332' };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#030A05' }}>
      ...
      <div style={{
        ...
        <p style={{ color: 'rgba(201,168,76,0.8)', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: '1rem' }}>
          ✨ مسنون دعائیں
        </p>
        <h1 style={{
        ...
        <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '1rem', maxWidth: 520, margin: '0 auto 0.5rem' }}>
          Authentic duas from the Quran and Sunnah for every moment of your day
        </p>
        <p style={{ color: 'rgba(201,168,76,0.6)', fontSize: '0.82rem', marginBottom: '2rem' }}>
      ...
      {!query && activeCategory === 'All' && !showFavOnly && dailyDua && (
        <div style={{ maxWidth: 860, margin: '-1.5rem auto 0', padding: '0 1.5rem', position: 'relative', zIndex: 2 }}>
          <div style={{
            background: 'linear-gradient(135deg, #0D2818 0%, #1B4332 60%, #2E7D32 100%)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
              <span style={{ background: 'rgba(201,168,76,0.2)', border: '1px solid rgba(201,168,76,0.4)', borderRadius: 99, padding: '0.25rem 0.75rem', fontSize: '0.7rem', fontWeight: 700, color: '#C9A84C', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                ⭐ آج کی دعا
              </span>
              <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>{dailyDua.category}</span>
            </div>
            <h2 style={{ color: '#C9A84C', fontFamily: '"Cinzel",serif', fontSize: '1.1rem', marginBottom: '1rem' }}>{dailyDua.title}</h2>
            <p style={{
              fontFamily: '"Noto Naskh Arabic","Amiri Quran",serif',
              fontSize: 'clamp(1.2rem, 3vw, 1.6rem)',
              lineHeight: 2.2, color: '#fff', textAlign: 'right', direction: 'rtl', marginBottom: '1rem',
            }}>{dailyDua.arabic}</p>
            <p style={{ fontStyle: 'italic', color: 'rgba(255,255,255,0.55)', fontSize: '0.82rem', marginBottom: '0.75rem' }}>{dailyDua.transliteration}</p>
            <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.85rem', lineHeight: 1.6 }}>{dailyDua.translation_en}</p>
            <div style={{ display: 'flex', gap: '0.7rem', marginTop: '1.2rem', flexWrap: 'wrap', alignItems: 'center' }}>
              <button onClick={() => speakArabic(dailyDua.arabic)} style={{
                ...
                borderRadius: 99, padding: '0.45rem 1rem', color: '#C9A84C',
                fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer',
              }}>🔊 سنیں</button>
              <button onClick={() => toggleFav(dailyDua.id)} style={{
                background: favorites.includes(dailyDua.id) ? 'rgba(229,57,53,0.15)' : 'rgba(255,255,255,0.08)',
              ...
              <span style={{ marginLeft: 'auto', background: 'rgba(255,255,255,0.06)', borderRadius: 99, padding: '0.45rem 0.9rem', fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>
                📖 {dailyDua.source}
              </span>
            </div>
          </div>
      ...
      <div ref={filterRef} style={{
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', gap: '0.5rem', overflowX: 'auto', paddingBottom: 2 }}>
          ...
          {CATEGORIES.map(cat => {
            const meta = CAT_META[cat] || { icon: '✨', color: '#1B4332' };
      ...
      {/* FOOTER BANNER */}
      <div style={{
        background: 'linear-gradient(135deg, #060E09, #1B4332)',
        ...
        <p style={{ fontFamily: '"Noto Naskh Arabic",serif', fontSize: '1.3rem', color: '#C9A84C', marginBottom: '0.5rem', direction: 'rtl' }}>
