  ...
  return (
    <div style={overlay}>
      <div style={modal}>
        ...
        {/* Progress bar */}
        <div style={{ height: 4, background: 'var(--cream-dark)', borderRadius: 2, marginBottom: '1.5rem' }}>
        ...
        <div
          ...
          style={{ fontFamily: 'var(--font-arabic)', fontSize: '4rem', textAlign: 'center', marginBottom: '1.5rem', cursor: 'pointer', padding: '1rem', background: 'var(--cream-dark)', borderRadius: 'var(--radius-lg)', userSelect: 'none' }}
          title="Tap to hear"
        >
          {q.arabic}
        </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: isActive ? 'var(--green-deep)' : 'var(--cream-dark)',
    ...
    return (
      <div>
        {lesson.non_connecting && (
          <div style={{ background: '#fff8e1', border: '1px solid var(--gold)', borderRadius: 'var(--radius)', padding: '0.75rem 1rem', marginBottom: '1.25rem', fontSize: '0.9rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>
          {lesson.examples?.map((ex, i) => (
            <div key={i} onClick={() => speak(ex.form || ex.isolated)} style={{ padding: '1rem', background: 'var(--cream-dark)', borderRadius: 'var(--radius)', border: '1px solid #ddd', cursor: 'pointer', transition: 'border-color 0.15s' }}
    ...
    return (
      <div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
          {lesson.examples?.map((ex, i) => (
            <div key={i} onClick={() => speak(ex.arabic)} style={{ padding: '1.25rem', borderRadius: 'var(--radius-lg)', border: `2px solid ${ex.color}`, background: 'var(--cream-dark)', cursor: 'pointer', transition: 'transform 0.1s' }}
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {lesson.practice.map((p, i) => (
                <div key={i} onClick={() => speak(p.arabic)} style={{ padding: '0.75rem 1rem', background: 'var(--cream-dark)', borderRadius: 'var(--radius)', border: '1px solid #ddd', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
  ...
  if (lesson.type === 'madd') {
    return (
      <div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
          {lesson.examples?.map((ex, i) => (
            <div key={i} onClick={() => speak(ex.arabic)} style={{ padding: '1.25rem', borderRadius: 'var(--radius-lg)', border: `2px solid ${ex.color}`, background: 'var(--cream-dark)', cursor: 'pointer' }}>
        ...
        {lesson.practice?.length > 0 && (
          <div>
            ...
            {lesson.practice.map((p, i) => (
              <div key={i} onClick={() => speak(p.arabic)} style={{ padding: '0.75rem 1rem', background: 'var(--cream-dark)', borderRadius: 'var(--radius)', border: '1px solid #ddd', cursor: 'pointer', marginBottom: '0.6rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
    ...
    return (
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.75rem' }}>
          {lesson.examples?.map((ex, i) => (
            <div key={i} onClick={() => speak(ex.arabic)} style={{ padding: '1rem', background: 'var(--cream-dark)', borderRadius: 'var(--radius)', border: '1px solid #ddd', cursor: 'pointer', textAlign: 'center' }}>
...
export default function QaidaPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 120px)', fontFamily: 'var(--font-body)' }}>


      <div style={{ display: 'flex', flex: 1 }}>

      {/* ── Sidebar ── */}
      <aside style={{ width: 240, flexShrink: 0, borderRight: '1px solid #e0ddd5', background: 'var(--cream-dark)', overflowY: 'auto', padding: '1rem 0' }}>
        ...
        {lessons.map(l => {
          ...
          return (
            <button key={l.id} onClick={() => setActiveId(l.id)} style={{
              ...
              <span style={{ width: 22, height: 22, borderRadius: '50%', background: done ? 'var(--success)' : isActive ? 'rgba(255,255,255,0.3)' : '#ddd', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', fontWeight: 'bold', color: done ? '#fff' : isActive ? '#fff' : 'var(--text-mid)', flexShrink: 0 }}>
                {done ? '✓' : l.id}
              </span>
