...
const LOG_TYPE_META = {
  sabaq:  { label: 'Sabaq',  icon: '📖', color: '#14532D', bg: '#E8F5E9' },
    ...
    <div style={{ minHeight: '100vh', background: '#FFFBEB' }}>
      ...
      <div style={{ background: 'linear-gradient(150deg,#052e16,#14532D,#14532D)', padding: '3rem 1.5rem 2.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative' }}>
          <p style={{ color: 'rgba(245,158,11,0.7)', fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: '0.5rem' }}>👨‍👩‍👦 لوحة الوالدين</p>
          ...
          <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.84rem', margin: 0 }}>Monitor your child's Quran memorization journey</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        {/* Link child panel */}
        <div style={{ background: '#fff', borderRadius: 18, border: '1px solid rgba(245,158,11,0.2)', boxShadow: '0 2px 12px rgba(20,83,45,0.07)', padding: '1.25rem 1.4rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '1.3rem' }}>🔗</span>
          <div style={{ flex: 1, minWidth: 200 }}>
            <p style={{ margin: '0 0 0.15rem', fontWeight: 700, fontSize: '0.85rem', color: '#14532D' }}>Link a Child Account</p>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
            <input value={linkCode} onChange={e => setLinkCode(e.target.value.toUpperCase())} placeholder="INVITE CODE"
              ...
              style={{ padding: '0.5rem 0.85rem', borderRadius: 8, border: '1.5px solid rgba(20,83,45,0.2)', fontFamily: 'monospace', fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.12em', color: '#14532D', background: '#F6F9F7', outline: 'none', width: 140 }} />
            <button onClick={linkChild} disabled={linking || !linkCode.trim()} style={{ padding: '0.5rem 1.1rem', background: 'linear-gradient(135deg,#14532D,#2E7D32)', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', opacity: linking ? 0.6 : 1 }}>
              {linking ? '...' : 'Link'}
            </button>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div style={{ background: '#fff', borderRadius: 20, border: '1px solid rgba(0,0,0,0.07)', padding: '4rem 2rem', textAlign: 'center', boxShadow: '0 2px 12px rgba(20,83,45,0.07)' }}>
          ...
          <>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', overflowX: 'auto' }}>
                {children.map(c => (
                  <button key={c.id} onClick={() => setSelectedChild(c)} style={{
                    ...
                    borderColor: (selectedChild?.id || children[0]?.id) === c.id ? '#14532D' : 'rgba(0,0,0,0.12)',
                    background: (selectedChild?.id || children[0]?.id) === c.id ? '#14532D' : '#fff',
                    ...
                    <div style={{ width: 24, height: 24, borderRadius: '50%', background: (selectedChild?.id || children[0]?.id) === c.id ? '#E7E5E4' : '#14532D15', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', fontWeight: 800 }}>{getInitials(c.name)}</div>
                    {c.name}
                  </button>
                ))}
              </div>
            ...
            {active && (
              <>
                {/* Child overview */}
                <div style={{ background: 'linear-gradient(135deg,#14532D,#14532D)', borderRadius: 20, padding: '1.75rem', marginBottom: '1.25rem', border: '1px solid rgba(245,158,11,0.2)', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem' }}>
                    <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg,#F59E0B,#A07830)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', fontWeight: 800, color: '#050E08', flexShrink: 0 }}>{getInitials(active.name)}</div>
                    <div>
                      <p style={{ margin: 0, fontFamily: '"Playfair Display",serif', fontSize: '1.1rem', color: '#fff', fontWeight: 700 }}>{active.name}</p>
                      <p style={{ margin: '0.15rem 0 0', fontSize: '0.72rem', color: '#A8A29E' }}>{active.email}</p>
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '0.75rem' }}>
                    {[
                      ...
                      <div key={i} style={{ textAlign: 'center', padding: '0.75rem', background: 'rgba(255,255,255,0.07)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.12)' }}>
                        <p style={{ margin: '0 0 0.2rem', fontSize: '1rem' }}>{s.icon}</p>
                        <p style={{ margin: 0, fontFamily: '"Playfair Display",serif', fontSize: '1.1rem', color: '#F59E0B', fontWeight: 800 }}>{s.val}</p>
                        <p style={{ margin: '0.1rem 0 0', fontSize: '0.62rem', color: '#A8A29E', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{s.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ background: '#fff', borderRadius: 18, border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 2px 12px rgba(20,83,45,0.07)', overflow: 'hidden' }}>
                  <div style={{ padding: '0.9rem 1.3rem', borderBottom: '1px solid rgba(0,0,0,0.05)', background: 'linear-gradient(to right,rgba(20,83,45,0.03),transparent)' }}>
                    <h3 style={{ margin: 0, fontSize: '0.88rem', fontWeight: 700, color: '#14532D' }}>Recent Sessions — {active.name}</h3>
                  </div>
                  {childLogs.length === 0 ? (
                    ...
                    return (
                      <div key={log.id} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {log.quality_rating && <span style={{ marginLeft: '0.5rem', fontSize: '0.65rem', color: '#F59E0B' }}>{'★'.repeat(log.quality_rating)}</span>}
                        </div>
