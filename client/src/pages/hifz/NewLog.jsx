...
const LOG_TYPES = [
  { value: 'sabaq',  label: 'Sabaq',  urdu: 'سبق',   icon: '📖', desc: 'New memorization today',    color: '#1B4332', bg: 'linear-gradient(135deg,#1B4332,#2E7D32)' },
      ...
      {[1, 2, 3, 4, 5].map(n => (
        <button key={n} type="button"
          ...
          style={{
            ...
            color: n <= (hover || value) ? '#C9A84C' : '#DDD',
    ...
    <div style={{ minHeight: '100vh', background: '#F6F1E9' }}>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 640, margin: '0 auto', position: 'relative' }}>
          <Link to="/hifz" style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.78rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.35rem', marginBottom: '1rem' }}>← Back to Hifz</Link>
          <h1 style={{ fontFamily: '"Cinzel",serif', fontSize: '1.6rem', color: '#fff', margin: 0 }}>Log Session</h1>
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.84rem', margin: '0.3rem 0 0' }}>Record your hifz progress for today</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '2rem 1.5rem' }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: '#fff', borderRadius: 20, border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 2px 12px rgba(27,67,50,0.07)', padding: '1.5rem', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.85rem' }}>
              <div>
                ...
                <input type="number" min="1" max="114" placeholder="1–114" value={form.surah_start} onChange={set('surah_start')} style={{ ...inputStyle, borderColor: errors.surah_start ? '#e53935' : 'rgba(27,67,50,0.18)' }} required
                  onFocus={e => e.target.style.borderColor = '#1B4332'} onBlur={e => e.target.style.borderColor = errors.surah_start ? '#e53935' : 'rgba(27,67,50,0.18)'} />
              ...
              <div>
                ...
                <input type="number" min="1" max={AYAH_COUNTS[parseInt(form.surah_start)-1] || 999} placeholder="Ayah #" value={form.ayah_start} onChange={set('ayah_start')} style={{ ...inputStyle, borderColor: errors.ayah_start ? '#e53935' : 'rgba(27,67,50,0.18)' }} required
                  onFocus={e => e.target.style.borderColor = '#1B4332'} onBlur={e => e.target.style.borderColor = errors.ayah_start ? '#e53935' : 'rgba(27,67,50,0.18)'} />
                {errors.ayah_start && <p style={errStyle}>{errors.ayah_start}</p>}
              </div>
              <div>
                <label style={labelStyle}>To Surah</label>
                <input type="number" min="1" max="114" placeholder="1–114" value={form.surah_end} onChange={set('surah_end')} style={{ ...inputStyle, borderColor: errors.surah_end ? '#e53935' : 'rgba(27,67,50,0.18)' }} required
                  onFocus={e => e.target.style.borderColor = '#1B4332'} onBlur={e => e.target.style.borderColor = errors.surah_end ? '#e53935' : 'rgba(27,67,50,0.18)'} />
              ...
              <div>
                ...
                <input type="number" min="1" max={AYAH_COUNTS[parseInt(form.surah_end)-1] || 999} placeholder="Ayah #" value={form.ayah_end} onChange={set('ayah_end')} style={{ ...inputStyle, borderColor: errors.ayah_end ? '#e53935' : 'rgba(27,67,50,0.18)' }} required
                  onFocus={e => e.target.style.borderColor = '#1B4332'} onBlur={e => e.target.style.borderColor = errors.ayah_end ? '#e53935' : 'rgba(27,67,50,0.18)'} />
          ...
          <button type="submit" disabled={loading} style={{
            ...
            background: loading ? '#ccc' : selectedType?.bg || 'linear-gradient(135deg,#1B4332,#2E7D32)',
