...
const LOG_TYPE_META = {
  sabaq:   { label: 'Sabaq',   color: '#1B4332', bg: '#E8F5E9', icon: '📖' },
  sabaqi:  { label: 'Sabaqi',  color: '#1565C0', bg: '#E3F2FD', icon: '🔄' },
  manzil:  { label: 'Manzil',  color: '#6A1B9A', bg: '#F3E5F5', icon: '⭐' },
  dhor:    { label: 'Dhor',    color: '#E65100', bg: '#FFF3E0', icon: '🎯' },
};
    ...
    <div style={{ minHeight: '100vh', background: '#F6F1E9' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
          {[
            { icon: '🔥', value: streak?.current_streak || 0, label: 'Day Streak', sub: 'Keep it going!', color: '#E65100' },
            { icon: '🏆', value: streak?.longest_streak || 0, label: 'Best Streak', sub: 'Personal record', color: '#C9A84C' },
            { icon: '📋', value: totalSessions, label: 'Total Sessions', sub: 'All time', color: '#1B4332' },
            { icon: '📅', value: todayCount, label: "Today's Logs", sub: new Date().toLocaleDateString('en-PK', { weekday: 'long' }), color: '#1565C0' },
          ].map((s, i) => (
            <div key={i} style={{
              ...
              <div style={{ width: 44, height: 44, borderRadius: 12, background: `${s.color}12`, border: `1px solid ${s.color}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.3rem', flexShrink: 0 }}>{s.icon}</div>
              <div>
                <p style={{ margin: 0, fontFamily: '"Cinzel",serif', fontSize: '1.4rem', fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.value}</p>
                <p style={{ margin: '0.15rem 0 0', fontSize: '0.75rem', fontWeight: 700, color: '#333' }}>{s.label}</p>
                <p style={{ margin: 0, fontSize: '0.65rem', color: '#aaa' }}>{s.sub}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          {/* Session breakdown */}
          <div style={{ background: '#fff', borderRadius: 18, padding: '1.4rem', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 2px 12px rgba(27,67,50,0.07)' }}>
            <p style={{ margin: '0 0 0.85rem', fontSize: '0.8rem', fontWeight: 700, color: '#666', textTransform: 'uppercase', letterSpacing: '0.1em' }}>📈 Session Types</p>
            {Object.entries(LOG_TYPE_META).map(([type, meta]) => {
              ...
              return (
                <div key={type} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.55rem' }}>
                  ...
                  <span style={{ fontSize: '0.68rem', color: '#aaa', width: 24, textAlign: 'right' }}>{count}</span>
                </div>
              );
            })}
          </div>
        ...
        {/* Recent Logs */}
        <div style={{ background: '#fff', borderRadius: 20, border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 2px 12px rgba(27,67,50,0.07)', overflow: 'hidden' }}>
          <div style={{ padding: '1rem 1.4rem', borderBottom: '1px solid rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(to right, rgba(27,67,50,0.03), transparent)' }}>
            <h3 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 700, color: '#1B4332' }}>Recent Sessions</h3>
            <Link to="/hifz/new-log" style={{ fontSize: '0.75rem', color: '#C9A84C', fontWeight: 700, textDecoration: 'none' }}>+ Add New</Link>
          </div>

          {isLoading ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: '#aaa', fontSize: '0.85rem' }}>Loading sessions...</div>
          ) : !(logs?.length) ? (
            <div style={{ padding: '3rem', textAlign: 'center' }}>
              <p style={{ fontSize: '2.5rem', marginBottom: '0.75rem' }}>📖</p>
              <p style={{ fontWeight: 700, color: '#555', fontSize: '0.95rem' }}>No sessions yet</p>
              <p style={{ color: '#aaa', fontSize: '0.82rem', marginBottom: '1.25rem' }}>Log your first sabaq to start tracking</p>
              <Link to="/hifz/new-log" style={{ padding: '0.6rem 1.4rem', background: 'linear-gradient(135deg, #1B4332, #2E7D32)', color: '#fff', borderRadius: 99, textDecoration: 'none', fontSize: '0.82rem', fontWeight: 700 }}>Log First Session</Link>
            </div>
          ) : (
            <div>
              {(logs || []).slice(0, 8).map((log, i) => {
                ...
                return (
                  <div key={log.id} style={{
                    ...
                    borderBottom: i < Math.min(logs.length, 8) - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none',
                    transition: 'background 0.15s',
                  }}
                  onMouseEnter={e => e.currentTarget.style.background = '#FAFAFA'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                  >
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: meta.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0 }}>{meta.icon}</div>
                    ...
                    <span style={{ fontSize: '0.7rem', color: '#bbb', whiteSpace: 'nowrap', flexShrink: 0 }}>{formatDate(log.log_date)}</span>
                  </div>
                );
              })}
            </div>
