...
const TYPE_META = {
  ...
  ayah:     { label: 'Ayah',     icon: '📖', color: '#22C55E' },
  other:    { label: 'Other',    icon: '⚠️', color: '#A8A29E' },
};
...
export default function MistakesPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FFFBEB' }}>
      ...
      {/* Header */}
      <div style={{ background: 'linear-gradient(150deg,#052e16,#14532D,#14532D)', padding: '2.5rem 1.5rem 2rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 860, margin: '0 auto', position: 'relative' }}>
          <Link to="/hifz" style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.78rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.35rem', marginBottom: '1rem' }}>← Hifz Home</Link>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              <h1 style={{ fontFamily: '"Playfair Display",serif', fontSize: 'clamp(1.4rem,3vw,2rem)', color: '#fff', margin: '0 0 0.3rem' }}>Mistakes Log</h1>
              <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.84rem', margin: 0 }}>Review, practice, and resolve your recitation mistakes</p>
            </div>
            <div style={{ display: 'flex', gap: '0.6rem' }}>
              <div style={{ background: 'rgba(229,57,53,0.15)', border: '1px solid rgba(229,57,53,0.4)', borderRadius: 99, padding: '0.4rem 0.9rem', textAlign: 'center' }}>
                ...
                <p style={{ margin: '0.1rem 0 0', fontSize: '0.62rem', color: '#A8A29E', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Pending</p>
              </div>
              <div style={{ background: 'rgba(46,125,50,0.2)', border: '1px solid rgba(46,125,50,0.4)', borderRadius: 99, padding: '0.4rem 0.9rem', textAlign: 'center' }}>
                ...
                <p style={{ margin: '0.1rem 0 0', fontSize: '0.62rem', color: '#A8A29E', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Resolved</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '1.5rem 1.5rem 3rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: '0.4rem' }}>
            {[['all', '📋 All', mistakes.length], ['pending', '⚠️ Pending', pending], ['resolved', '✅ Resolved', resolved]].map(([v, label, count]) => (
              <button key={v} onClick={() => setFilter(v)} style={{
                ...
                borderColor: filter === v ? 'rgba(245,158,11,0.5)' : 'rgba(28,25,23,0.1)',
                background: filter === v ? 'linear-gradient(135deg,#F59E0B,#FCD34D)' : 'rgba(28,25,23,0.04)',
                color: filter === v ? '#FFFBEB' : '#78716C',
          ...
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{
            padding: '0.4rem 0.75rem', borderRadius: 8, border: '1px solid rgba(28,25,23,0.1)',
            background: '#fff', fontSize: '0.75rem', color: '#57534E', cursor: 'pointer', outline: 'none',
          }}>
        ...
        {isLoading ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#78716C' }}>Loading mistakes...</div>
        ) : mistakes.length === 0 ? (
          <div style={{ background: '#ffffff', borderRadius: 20, border: '1px solid rgba(28,25,23,0.09)', padding: '4rem 2rem', textAlign: 'center' }}>
            <p style={{ fontSize: '3rem', marginBottom: '0.75rem' }}>🎉</p>
            <p style={{ fontWeight: 700, fontSize: '1.1rem', color: 'rgba(82,183,136,0.85)' }}>No mistakes recorded!</p>
            <p style={{ color: '#78716C', fontSize: '0.85rem', marginBottom: '1.5rem' }}>Your teachers can log mistakes during your revision sessions</p>
            <Link to="/hifz" style={{ padding: '0.65rem 1.5rem', background: 'linear-gradient(135deg,#14532D,#15803D)', color: '#fff', borderRadius: 99, textDecoration: 'none', fontSize: '0.85rem', fontWeight: 700 }}>Back to Hifz</Link>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#78716C' }}>No {filter} mistakes found.</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {grouped.map(({ surah, items }) => (
              <div key={surah} style={{ background: '#ffffff', borderRadius: 18, border: '1px solid rgba(28,25,23,0.09)', overflow: 'hidden' }}>
                <div style={{ padding: '0.85rem 1.25rem', background: 'rgba(245,158,11,0.04)', borderBottom: '1px solid rgba(28,25,23,0.07)', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: 'linear-gradient(135deg,#14532D,#15803D)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', color: '#F59E0B', fontWeight: 800 }}>{surah}</div>
                  <div>
                    <p style={{ margin: 0, fontWeight: 700, fontSize: '0.88rem', color: 'rgba(245,158,11,0.85)' }}>{SURAH_NAMES[surah - 1] || `Surah ${surah}`}</p>
                    <p style={{ margin: 0, fontSize: '0.68rem', color: '#78716C' }}>{items.length} mistake{items.length !== 1 ? 's' : ''}</p>
                  </div>
                  <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.4rem' }}>
                    ...
                    {items.filter(i => i.resolved).length > 0 && <span style={{ fontSize: '0.65rem', background: 'rgba(46,125,50,0.15)', color: '#22C55E', borderRadius: 99, padding: '0.2rem 0.55rem', fontWeight: 700 }}>{items.filter(i => i.resolved).length} resolved</span>}
                  </div>
                </div>
                ...
                {items.map((m, i) => {
                  ...
                  return (
                    <div key={m.id} style={{
                      ...
                      borderBottom: i < items.length - 1 ? '1px solid rgba(28,25,23,0.07)' : 'none',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.2rem' }}>
                          <span style={{ fontWeight: 700, fontSize: '0.82rem', color: '#292524' }}>Ayah {m.ayah_num}</span>
                          ...
                          {m.resolved && <span style={{ fontSize: '0.68rem', color: '#22C55E', fontWeight: 600 }}>Resolved</span>}
                        </div>
                        {m.description && <p style={{ margin: 0, fontSize: '0.8rem', color: '#A8A29E', lineHeight: 1.6 }}>{m.description}</p>}
                      </div>
                      {!m.resolved && (
