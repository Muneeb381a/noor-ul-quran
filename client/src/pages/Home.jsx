...
function IslamicStar({ size = 400, opacity = 0.04, style = {} }) {
  return (
    <svg width={size} height={size} viewBox="0 0 200 200"
      ...
      <circle cx="100" cy="100" r="38" fill="none" stroke="rgba(201,168,76,1)" strokeWidth="0.3" opacity={opacity} />
    </svg>
  );
}
...
const MODULES = [
  { href:'/quran',       icon:'📖', arabic:'القرآن الكريم',    title:'Quran Reader',   desc:'Mushaf-style layout with Tajweed text, dual translation (English + Urdu), ayah markers, and bookmarks.', badge:'114 Surahs', tag:'Core',         gradFrom:'#051408', gradTo:'#0D2818', accent:'#52B788', steps:['Mushaf layout','Dual translation','Audio TTS','Bookmarks'] },
...
const FEATURES = [
  { icon:'🕌', title:'Amiri Quran Font',  desc:'Uthmani script identical to Madinah Mushaf, rendered perfectly at any size',          color:'#52B788' },
  { icon:'🔊', title:'Arabic Audio TTS',  desc:'Correct pronunciation for every letter and ayah via our server proxy',                color:'#64B5F6' },
  { icon:'🌍', title:'Dual Translation',  desc:'Sahih International + Jalandhry Urdu translations, always visible side by side',      color:'#E9C46A' },
...
const STEPS = [
  { n:'01', icon:'📝', title:'Create Account',   desc:'Sign up in seconds — no credit card required, free forever.',      color:'#52B788' },
  { n:'02', icon:'✏️', title:'Start with Qaida', desc:'Master the Arabic alphabet with audio, diagrams and quizzes.',     color:'#64B5F6' },
  { n:'03', icon:'📖', title:'Read the Quran',   desc:'Explore 114 surahs with Tajweed text and dual translation.',       color:'#E9C46A' },
...
export default function Home() {
  ...
  return (
    <div style={{ overflowX: 'hidden', background: '#030A05' }}>

      {/* ══════════════════════════ 1. HERO ══════════════════════════════ */}
      <section style={{
        minHeight: '100vh', position: 'relative', overflow: 'hidden',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        background: 'linear-gradient(175deg,#020806 0%,#060F08 28%,#0A1C10 62%,#0D2818 100%)',
        ...
        <svg aria-hidden style={{ position:'absolute',inset:0,width:'100%',height:'100%',opacity:0.025 }}>
          <defs><pattern id="hx" x="0" y="0" width="56" height="64" patternUnits="userSpaceOnUse">
            <polygon points="28,2 52,16 52,48 28,62 4,48 4,16" fill="none" stroke="#C9A84C" strokeWidth="0.8"/>
          </pattern></defs>
        ...
        <div style={{ position:'absolute',top:'32%',right:'6%',width:620,height:620,borderRadius:'50%',background:'radial-gradient(circle,rgba(201,168,76,0.06) 0%,transparent 65%)',pointerEvents:'none',filter:'blur(65px)' }}/>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ width:6,height:6,borderRadius:'50%',background:'#52B788',display:'inline-block',animation:'pulseGold 2.2s ease-in-out infinite' }}/>
          ...
          <p style={{
            ...
            color:'rgba(201,168,76,0.55)',direction:'rtl',lineHeight:2.5,
          ...
          <h1 style={{
            ...
            background:'linear-gradient(155deg,#FFFCF0 0%,#F5DFA0 20%,#E9C46A 48%,#C9A84C 70%,#F5DFA0 100%)',
            WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',
            filter:'drop-shadow(0 8px 44px rgba(201,168,76,0.28))',
          }}>نور القرآن</h1>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:'1rem',justifyContent:'center',marginBottom:'1.8rem' }}>
            <div style={{ width:75,height:1,background:'linear-gradient(to left,rgba(201,168,76,0.4),transparent)' }}/>
            <span style={{ color:'rgba(201,168,76,0.35)',fontSize:'0.48rem' }}>◆</span>
            <span style={{ color:'rgba(201,168,76,0.65)',fontSize:'0.85rem' }}>✦</span>
            <span style={{ color:'rgba(201,168,76,0.35)',fontSize:'0.48rem' }}>◆</span>
            <div style={{ width:75,height:1,background:'linear-gradient(to right,rgba(201,168,76,0.4),transparent)' }}/>
          </div>
          ...
          <div style={{ display:'flex',gap:'0.85rem',justifyContent:'center',flexWrap:'wrap',marginBottom:'3.5rem' }}>
            <Link to="/quran" style={{
              ...
              background:'linear-gradient(135deg,#E2B84E 0%,#C9A84C 55%,#A8810A 100%)',
              ...
              boxShadow:'0 0 0 1px rgba(255,255,255,0.18) inset,0 4px 32px rgba(201,168,76,0.55)',
              transition:'all 0.3s cubic-bezier(0.22,1,0.36,1)',textDecoration:'none',
              display:'inline-flex',alignItems:'center',gap:'0.5rem',
            }}
            onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-3px) scale(1.02)';e.currentTarget.style.boxShadow='0 0 0 1px rgba(255,255,255,0.22) inset,0 10px 50px rgba(201,168,76,0.7)';}}
            onMouseLeave={e=>{e.currentTarget.style.transform='';e.currentTarget.style.boxShadow='0 0 0 1px rgba(255,255,255,0.18) inset,0 4px 32px rgba(201,168,76,0.55)';}}
            ...
            <Link to="/register" style={{
              ...
              color:'rgba(255,255,255,0.82)',borderRadius:99,backdropFilter:'blur(12px)',
              transition:'all 0.28s',textDecoration:'none',
              display:'inline-flex',alignItems:'center',gap:'0.4rem',
            }}
            onMouseEnter={e=>{e.currentTarget.style.background='rgba(255,255,255,0.09)';e.currentTarget.style.borderColor='rgba(201,168,76,0.45)';e.currentTarget.style.color='#E9C46A';e.currentTarget.style.transform='translateY(-2px)';}}
            onMouseLeave={e=>{e.currentTarget.style.background='rgba(255,255,255,0.045)';e.currentTarget.style.borderColor='rgba(255,255,255,0.15)';e.currentTarget.style.color='rgba(255,255,255,0.82)';e.currentTarget.style.transform='';}}
            >Start for Free →</Link>
          </div>
          ...
          <div style={{ display:'flex',gap:'0.55rem',justifyContent:'center',flexWrap:'wrap' }}>
            {[
              ...
              onMouseEnter={e=>{e.currentTarget.style.background='rgba(201,168,76,0.09)';e.currentTarget.style.borderColor='rgba(201,168,76,0.3)';e.currentTarget.style.color='#E9C46A';}}
              onMouseLeave={e=>{e.currentTarget.style.background='rgba(255,255,255,0.04)';e.currentTarget.style.borderColor='rgba(255,255,255,0.1)';e.currentTarget.style.color='rgba(255,255,255,0.48)';}}
              >
                <span>{s.icon}</span>
                <strong style={{ color:'rgba(255,255,255,0.88)',fontWeight:700 }}>{s.val}</strong>
                <span>{s.label}</span>
              </div>
            ))}
          </div>
      ...
      {/* ══════════════════════════ 2. TICKER ════════════════════════════ */}
      <div style={{ background:'linear-gradient(90deg,#0A1F12,#0D2818,#0A1F12)',borderTop:'1px solid rgba(201,168,76,0.1)',borderBottom:'1px solid rgba(201,168,76,0.1)',padding:'0.65rem 0',overflow:'hidden' }}>
        <div style={{ display:'flex',animation:'marqueeScroll 30s linear infinite',width:'max-content' }}>
          {[...TICKER,...TICKER].map((t,i) => (
            <span key={i} style={{ display:'inline-flex',alignItems:'center',gap:'0.45rem',padding:'0 2.2rem',fontSize:'0.77rem',color:'rgba(255,255,255,0.4)',fontWeight:500,whiteSpace:'nowrap' }}>
              {t}<span style={{ color:'rgba(201,168,76,0.28)' }}>✦</span>
            </span>
          ))}
        </div>
      </div>
      ...
      {/* ══════════════════════════ 3. STATS ═════════════════════════════ */}
      <section style={{ background:'#060F08',padding:'4.5rem 1.5rem' }}>
        <div className="home-stats" style={{ maxWidth:1020,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem' }}>
          {[
            ...
            onMouseEnter={e=>{e.currentTarget.style.background=`rgba(${s.rgb},0.07)`;e.currentTarget.style.borderColor=`rgba(${s.rgb},0.22)`;e.currentTarget.style.transform='translateY(-5px)';}}
            onMouseLeave={e=>{e.currentTarget.style.background='rgba(255,255,255,0.025)';e.currentTarget.style.borderColor='rgba(255,255,255,0.07)';e.currentTarget.style.transform='';}}
            >
              <div style={{ fontSize:'1.85rem',marginBottom:'0.72rem' }}>{s.icon}</div>
              <div style={{ fontFamily:'var(--font-heading)',fontSize:'clamp(2rem,4vw,2.9rem)',fontWeight:800,lineHeight:1,background:`linear-gradient(135deg,#fff 0%,rgba(${s.rgb},0.92) 100%)`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text' }}>
                {s.val.toLocaleString()}{s.s}
              </div>
              <div style={{ fontWeight:700,fontSize:'0.88rem',color:'rgba(255,255,255,0.82)',marginTop:'0.5rem' }}>{s.label}</div>
              <div style={{ fontSize:'0.68rem',color:'rgba(255,255,255,0.28)',marginTop:'0.2rem' }}>{s.sub}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ══════════════════════════ 4. MODULES ═══════════════════════════ */}
      <section style={{ background:'#030A05',padding:'5.5rem 1.5rem',borderTop:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth:1180,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:'3.5rem' }}>
            <div style={{ display:'inline-flex',alignItems:'center',gap:'0.4rem',padding:'0.28rem 0.9rem',borderRadius:99,marginBottom:'0.85rem',background:'rgba(201,168,76,0.08)',border:'1px solid rgba(201,168,76,0.22)',fontSize:'0.7rem',fontWeight:700,letterSpacing:'0.13em',textTransform:'uppercase',color:'rgba(201,168,76,0.72)' }}>✦ What&apos;s Inside</div>
            <h2 style={{ fontFamily:'var(--font-heading)',fontSize:'clamp(1.8rem,3.5vw,2.8rem)',fontWeight:700,background:WG,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',marginBottom:'0.75rem',lineHeight:1.25 }}>
              One Platform, Complete Journey
            </h2>
            <p style={{ color:'rgba(255,255,255,0.32)',fontSize:'1rem',maxWidth:480,margin:'0 auto' }}>From your first Alif to completing your Quran memorization</p>
          </div>
          ...
          <div className="home-modules" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1.1rem' }}>
            {MODULES.map(m => (
              <Link key={m.href} to={m.href} style={{
                ...
                border:'1px solid rgba(255,255,255,0.06)',boxShadow:'0 4px 28px rgba(0,0,0,0.32)',
              ...
              onMouseLeave={e=>{e.currentTarget.style.transform='';e.currentTarget.style.boxShadow='0 4px 28px rgba(0,0,0,0.32)';e.currentTarget.style.borderColor='rgba(255,255,255,0.06)';}}
              >
      ...
      {/* ══════════════════════ 5. VERSE SHOWCASE ════════════════════════ */}
      <section style={{ position:'relative',padding:'7rem 1.5rem',overflow:'hidden',background:'linear-gradient(150deg,#060E09 0%,#0D2818 55%,#0A1C10 100%)' }}>
        <div style={{ position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 50%,rgba(201,168,76,0.07) 0%,transparent 62%)',pointerEvents:'none' }}/>
        ...
        <div style={{ maxWidth:760,margin:'0 auto',textAlign:'center',position:'relative',zIndex:2 }}>
          <div style={{ display:'inline-flex',alignItems:'center',gap:'0.5rem',padding:'0.3rem 1rem',borderRadius:99,marginBottom:'2rem',background:'rgba(201,168,76,0.08)',border:'1px solid rgba(201,168,76,0.22)',fontSize:'0.7rem',fontWeight:700,letterSpacing:'0.14em',textTransform:'uppercase',color:'rgba(201,168,76,0.72)' }}>
            ✦ Verse of the Day
          </div>

          {/* Framed ayah */}
          <div style={{ background:'rgba(201,168,76,0.035)',border:'1px solid rgba(201,168,76,0.12)',borderRadius:26,padding:'3rem 2.5rem',marginBottom:'2rem',position:'relative' }}>
            {/* Corner ornaments */}
            {[
              { top:10,left:10,  borderTop:'1.5px solid rgba(201,168,76,0.5)',borderLeft:'1.5px solid rgba(201,168,76,0.5)',  borderRadius:'12px 0 0 0' },
              { top:10,right:10, borderTop:'1.5px solid rgba(201,168,76,0.5)',borderRight:'1.5px solid rgba(201,168,76,0.5)', borderRadius:'0 12px 0 0' },
              { bottom:10,right:10,borderBottom:'1.5px solid rgba(201,168,76,0.5)',borderRight:'1.5px solid rgba(201,168,76,0.5)',borderRadius:'0 0 12px 0' },
              { bottom:10,left:10, borderBottom:'1.5px solid rgba(201,168,76,0.5)',borderLeft:'1.5px solid rgba(201,168,76,0.5)', borderRadius:'0 0 0 12px' },
            ].map((c,i) => <div key={i} style={{ position:'absolute',width:28,height:28,...c }}/>)}
            <p style={{ fontFamily:'var(--font-quran)',fontSize:'clamp(1.6rem,4.5vw,3rem)',color:'rgba(255,255,255,0.92)',direction:'rtl',lineHeight:2.4,animation:'glowPulse 5s ease-in-out infinite' }}>
              وَرَتِّلِ ٱلۡقُرۡءَانَ تَرۡتِيلٗا
            </p>
          </div>

          <div style={{ width:48,height:1,background:'rgba(201,168,76,0.28)',margin:'0 auto 1.25rem' }}/>
          ...
          <p style={{ color:'rgba(201,168,76,0.42)',fontSize:'0.78rem',marginBottom:'2.5rem' }}>— Surah Al-Muzzammil 73:4</p>

          <Link to="/quran" style={{
            display:'inline-flex',alignItems:'center',gap:'0.5rem',padding:'0.82rem 2rem',
            background:'rgba(201,168,76,0.08)',border:'1.5px solid rgba(201,168,76,0.28)',
            color:'#E9C46A',borderRadius:99,fontWeight:600,fontSize:'0.88rem',
            transition:'all 0.25s',textDecoration:'none',
          }}
          onMouseEnter={e=>{e.currentTarget.style.background='rgba(201,168,76,0.16)';e.currentTarget.style.borderColor='rgba(201,168,76,0.55)';e.currentTarget.style.transform='translateY(-2px)';}}
          onMouseLeave={e=>{e.currentTarget.style.background='rgba(201,168,76,0.08)';e.currentTarget.style.borderColor='rgba(201,168,76,0.28)';e.currentTarget.style.transform='';}}
          >📖 Open Quran Reader</Link>
        </div>
      </section>

      {/* ══════════════════════════ 6. FEATURES ══════════════════════════ */}
      <section style={{ background:'#060F08',padding:'5.5rem 1.5rem',borderTop:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:'3.5rem' }}>
            ...
            <h2 style={{ fontFamily:'var(--font-heading)',fontSize:'clamp(1.7rem,3vw,2.5rem)',fontWeight:700,background:WG,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',marginBottom:'0.6rem' }}>Built for Every Muslim</h2>
            <p style={{ color:'rgba(255,255,255,0.3)',fontSize:'0.95rem' }}>Everything you need, nothing you don&apos;t</p>
          </div>
          ...
          <div className="home-features" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1rem' }}>
            {FEATURES.map((f,i) => (
              ...
              onMouseEnter={e=>{e.currentTarget.style.background='rgba(255,255,255,0.04)';e.currentTarget.style.borderColor=`${f.color}28`;e.currentTarget.style.transform='translateY(-5px)';}}
              ...
              >
                ...
                <h3 style={{ fontSize:'0.93rem',color:'rgba(255,255,255,0.88)',fontWeight:700,marginBottom:'0.5rem',fontFamily:'var(--font-heading)' }}>{f.title}</h3>
                <p style={{ fontSize:'0.8rem',color:'rgba(255,255,255,0.36)',lineHeight:1.75 }}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background:'#030A05',padding:'5.5rem 1.5rem',borderTop:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth:980,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:'3.5rem' }}>
            ...
            <h2 style={{ fontFamily:'var(--font-heading)',fontSize:'clamp(1.7rem,3vw,2.4rem)',fontWeight:700,background:WG,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text' }}>Begin in 4 Simple Steps</h2>
          </div>

          <div className="home-steps" style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem',position:'relative' }}>
            {/* Connecting line */}
            <div style={{ position:'absolute',top:36,left:'12.5%',right:'12.5%',height:1,background:'linear-gradient(90deg,transparent,rgba(201,168,76,0.2),rgba(201,168,76,0.2),transparent)',zIndex:0 }}/>
            {STEPS.map((s,i) => (
              <div key={i} style={{ textAlign:'center',padding:'1.75rem 1rem',position:'relative',zIndex:1 }}>
                <div style={{
                  ...
                  boxShadow:`0 0 0 6px #030A05,0 0 24px ${s.color}22`,
                }}>{s.icon}</div>
                <div style={{ fontFamily:'var(--font-heading)',fontSize:'0.58rem',letterSpacing:'0.18em',color:s.color,fontWeight:700,marginBottom:'0.4rem',opacity:0.72 }}>STEP {s.n}</div>
                <h3 style={{ fontSize:'0.92rem',color:'rgba(255,255,255,0.85)',fontWeight:700,marginBottom:'0.5rem',fontFamily:'var(--font-heading)' }}>{s.title}</h3>
                <p style={{ fontSize:'0.78rem',color:'rgba(255,255,255,0.33)',lineHeight:1.72 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ══════════════════════════ 8. FINAL CTA ═════════════════════════ */}
      <section style={{ position:'relative',padding:'7rem 1.5rem',textAlign:'center',overflow:'hidden',background:'linear-gradient(150deg,#060E09 0%,#0D2818 55%,#081510 100%)',borderTop:'1px solid rgba(201,168,76,0.12)' }}>
        <div style={{ position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 50%,rgba(201,168,76,0.065) 0%,transparent 62%)',pointerEvents:'none' }}/>
        ...
        <div style={{ maxWidth:640,margin:'0 auto',position:'relative',zIndex:2 }}>
          <div style={{ border:'1px solid rgba(201,168,76,0.14)',borderRadius:28,padding:'4rem 3rem',position:'relative',background:'rgba(201,168,76,0.025)' }}>
            {/* Corner ornaments */}
            {[
              { top:10,left:10,  borderTop:'2px solid rgba(201,168,76,0.48)',borderLeft:'2px solid rgba(201,168,76,0.48)',  borderRadius:'22px 0 0 0' },
              { top:10,right:10, borderTop:'2px solid rgba(201,168,76,0.48)',borderRight:'2px solid rgba(201,168,76,0.48)', borderRadius:'0 22px 0 0' },
              { bottom:10,right:10,borderBottom:'2px solid rgba(201,168,76,0.48)',borderRight:'2px solid rgba(201,168,76,0.48)',borderRadius:'0 0 22px 0' },
              { bottom:10,left:10, borderBottom:'2px solid rgba(201,168,76,0.48)',borderLeft:'2px solid rgba(201,168,76,0.48)', borderRadius:'0 0 0 22px' },
            ].map((c,i) => <div key={i} style={{ position:'absolute',width:32,height:32,...c }}/>)}

            <p style={{ fontFamily:'var(--font-quran)',fontSize:'clamp(1.15rem,2.6vw,1.85rem)',color:'rgba(201,168,76,0.65)',direction:'rtl',lineHeight:2.4,marginBottom:'1rem',animation:'glowPulse 5s ease-in-out infinite' }}>
            ...
            <p style={{ color:'rgba(201,168,76,0.35)',fontSize:'0.74rem',marginBottom:'2.5rem' }}>— Al-Isra 17:9</p>
            ...
            <p style={{ color:'rgba(255,255,255,0.3)',marginBottom:'2.5rem',fontSize:'0.88rem' }}>
              Free forever &nbsp;·&nbsp; No sign-up required to start reading
            </p>

            <div style={{ display:'flex',gap:'0.85rem',justifyContent:'center',flexWrap:'wrap' }}>
              <Link to="/register" style={{
                padding:'0.9rem 2.6rem',background:'linear-gradient(135deg,#E2B84E,#C9A84C)',
              ...
              onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-3px)';e.currentTarget.style.boxShadow='0 8px 50px rgba(201,168,76,0.65)';}}
              onMouseLeave={e=>{e.currentTarget.style.transform='';e.currentTarget.style.boxShadow='0 4px 32px rgba(201,168,76,0.45)';}}
              >Create Free Account</Link>

              <Link to="/quran" style={{
                padding:'0.9rem 2.2rem',background:'transparent',
                color:'rgba(255,255,255,0.68)',border:'1px solid rgba(255,255,255,0.15)',
                borderRadius:99,fontWeight:600,fontSize:'0.9rem',
                transition:'all 0.22s',textDecoration:'none',display:'inline-block',
              }}
              onMouseEnter={e=>{e.currentTarget.style.background='rgba(255,255,255,0.07)';e.currentTarget.style.borderColor='rgba(255,255,255,0.38)';e.currentTarget.style.color='#fff';}}
